...
import softsugar.senseme.com.effects.encoder.mediacodec.utils.VideoInfo;
...
public class VideoProcessor {
    ...
    private MediaCodec videoDecoder2, videoDecoder3;
    private boolean mVideoFormatMp4vEs = false;
    private boolean mVideoFormatMp4Hevc = false;
    ...
    public VideoProcessor(Context context) {
        ...
        try {
            ...
            videoDecoder2 = MediaCodec.createDecoderByType("video/mp4v-es");
            videoDecoder3 = MediaCodec.createDecoderByType("video/hevc");
    ...
    public void processVideo(long startPosition, long clipDur) throws IOException {
        ...
        for (int i = 0; i < mVideoExtractor.getTrackCount(); i++) {
            ...
            if (format.getString(MediaFormat.KEY_MIME).startsWith("video/")) {
                videoTrackIndex = i;
                videoFormat = format;
                if(format.getString(MediaFormat.KEY_MIME).contains("mp4v-es")){
                    mVideoFormatMp4vEs = true;
                }
                if(format.getString(MediaFormat.KEY_MIME).contains("hevc")){
                    mVideoFormatMp4Hevc = true;
                }
                continue;
            }
...
//                muxAudioTrack = mMediaMuxer.addTrack(format);
    ...
    private Runnable videoCliper = new Runnable() {
        ...
        public void run() {
            ...
            if(mVideoEncodeInitSucceed){
                if(mVideoFormatMp4vEs){
                    startVideoCodec(videoDecoder2, videoEncoder, mVideoExtractor, inputSurface, outputSurface, firstVideoTime, startPosition, clipDur);
                }else if(mVideoFormatMp4Hevc){
                    startVideoCodec(videoDecoder3, videoEncoder, mVideoExtractor, inputSurface, outputSurface, firstVideoTime, startPosition, clipDur);
                }else {
                ...
                VideoProcessor.this.listener.onFailed();
                forceRelease();
            }
    ...
    private Runnable audioCliper = new Runnable() {
        ...
        public void run() {
            ...
            if(audioTrackIndex >= 0 && Build.VERSION.SDK_INT >= Constants.ANDROID_MIN_SDK_VERSION){
                mNeedAudio = true;
            }
    ...
    private boolean initAudioCodec() {
        try{
            ...
            Log.e("softsugar", "initAudioCodec: "+e.getMessage() );
...
//                            }
    ...
    private boolean initVideoCodec() {
        ...
        outputSurface.setCurrentStickerPath(mCurrentStickerMaps);
        ...
        outputSurface.setGreenParamsMap(mGreenParamsMap);

        try{
            //解码器启动
            if(mVideoFormatMp4vEs){
                videoDecoder2.configure(videoFormat, outputSurface.getSurface(), null, 0);
                videoDecoder2.start();
            }else if(mVideoFormatMp4Hevc){
                videoDecoder3.configure(videoFormat, outputSurface.getSurface(), null, 0);
                videoDecoder3.start();
            }else {
            ...
            Log.e("MediaCodec", "Failed to start MediaCodec", e);
            return false;
        }
    ...
    private synchronized void release() {
        ...
        if(!mVideoEncodeInitSucceed){
            if(mNeedStop){
                if (listener != null) {
                    LogUtils.iTag(TAG, "release() called111");
                    listener.onCanceled();
                }
            }else {
                if (listener != null) {
            ...
            if(mNeedStop){
                if (listener != null) {
                    LogUtils.iTag(TAG, "release() called1122222221");
                    listener.onCanceled();
                }
            }else {
                if (listener != null) {
    ...
    public void setCurrentSticker(LinkedHashMap<Integer, String> map){
        mCurrentStickerMaps = map;
    }

    private HashMap<Integer, Float> mGreenParamsMap;
    public void setGreenParams(HashMap<Integer, Float> map){
        mGreenParamsMap = map;
    }
