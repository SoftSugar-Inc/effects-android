...
public class STLicenseUtils {
    ...
    public synchronized static boolean checkLicense(final Context context){
        if(USING_SERVER_LICENSE){
            LogUtils.file("start checkLicense from Server");
            return checkLicenseFromServer(context);
        }else{
            LogUtils.file("start checkLicense Local");
    ...
    */
    ...
    public static boolean checkLicenseFromBuffer(Context context, byte[] licBuffer, boolean isOnlineLicense){
        LogUtils.file("checkLicenseFromBuffer() checkLicenseFromBuffer isOnlineLicense:" + isOnlineLicense);
        if(licBuffer == null){
            LogUtils.file("licBuffer is null");
            Log.e(TAG, "checkLicenseFromBuffer: licBuffer is null ");
            return false;
        }
        LogUtils.file(Utils.getApp().getString(R.string.log_lic) + new String(licBuffer));
        ...
        if (activateCode == null|| (STMobileAuthentificationNative.checkActiveCodeFromBuffer(context, licenseBuffer, licenseBuffer.length(), activateCode, activateCode.length()) != 0)) {
            ...
            if(isOnlineLicense){
                ...
                LogUtils.file("checkLicenseFromBuffer() start generateActiveCodeFromBuffer" + "licenseBuffer.length():" + licenseBuffer.length());
                activateCode = STMobileAuthentificationNative.generateActiveCodeFromBuffer(context, licenseBuffer, licenseBuffer.length());
                LogUtils.file("checkLicenseFromBuffer() end generateActiveCodeFromBuffer" + activateCode);
            }
            if (activateCode != null && activateCode.length() >0) {
                SharedPreferences.Editor editor = sp.edit();
                editor.putString(PREF_ACTIVATE_CODE, activateCode);
            ...
            LogUtils.e(TAG, "generate license error: " + error);
            LogUtils.file("checkLicenseFromBuffer() generate license error:" + activateCode);
            return false;
        }
        LogUtils.file("checkLicenseFromBuffer() activeCode:" + activateCode + ", license success");
    ...
    public static boolean checkLicenseFromServer(final Context context){
        ...
        LogUtils.i(Utils.getApp().getString(R.string.log_lic_ret) + mCheckLicenseRet);
