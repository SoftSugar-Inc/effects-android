...
public class EffectInfoDataHelper implements IEffectInfoDataHelper {
    ...
    public long getBasicStamp() {
        return SpUtils.getLong(SP_BASIC_STAMP + mType.name(), 0L);
    }
    ...
    public float getStrength(EffectType type) {
        float strength = (float) SpUtils.getParam(type, mType.name() + SP_STRENGTH, -2f);
        Log.i(TAG, "type=" + type.getDesc() + ", strength=" + strength);
        return strength;
    }
    ...
    public void setStrength(EffectType type, float strength) {
        ...
        SpUtils.setParam(SP_BASIC_STAMP + mType.name(), System.currentTimeMillis());
        setBasicEffectStamp(type);
    }

    @Override
    public void setBasicEffectStamp(EffectType type) {
        SpUtils.setParam(type,mType.name()+"_stamp", System.currentTimeMillis());
    }

    @Override
    public long getBasicEffectStamp(EffectType type) {
        long strength = (long) SpUtils.getParam(type,mType.name()+"_stamp", 0L);
        return strength;
    }
    ...
    public static String SP_BASIC_STAMP = "sp_basic_stamp";
