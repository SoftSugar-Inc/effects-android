...
//		android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
		...
		if (DEBUG) LogUtils.iTag(TAG, "Encoder thread exiting");
        synchronized (mSync) {
        	mRequestStop = true;
            mIsCapturing = false;
        }
        ...
        while (mIsCapturing) {
			...
			if (inputBufferIndex >= 0) {
	            ...
	            if (length <= 0) {
	            	// send EOS
	            	mIsEOS = true;
	            	if (DEBUG) LogUtils.iTag(TAG, "send BUFFER_FLAG_END_OF_STREAM");
...
LOOP:	while (mIsCapturing) {
	...
	if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
					...
					if (DEBUG) LogUtils.iTag(TAG, "drain:BUFFER_FLAG_CODEC_CONFIG");
					mBufferInfo.size = 0;
                }
