package softsugar.senseme.com.effects.display.glutils;
...
public class STGLRender {
    ...
    private void initInner(int width, int height, int widthResize, int heightResize) {
        LogUtils.iTag(TAG, "initInner() called with: width = [" + width + "], height = [" + height + "], widthResize = [" + widthResize + "], heightResize = [" + heightResize + "]");
    ...
    public void adjustVideoTextureBuffer2(int orientation,boolean flipHorizontal, boolean flipVertical) {
        float[] textureCords = TextureRotationUtil.getVideoRotation(orientation, flipHorizontal, flipVertical);
        LogUtils.d(TAG, "==========rotation: " + orientation + " flipVertical: " + flipVertical
                + " texturePos: " + Arrays.toString(textureCords));
        if (mTextureBuffer2 == null) {
            mTextureBuffer2 = ByteBuffer.allocateDirect(textureCords.length * 4)
                    .order(ByteOrder.nativeOrder())
                    .asFloatBuffer();
        }
        mTextureBuffer2.clear();
        mTextureBuffer2.put(textureCords).position(0);
    }
    ...
    public void calculateVertexBuffer(int displayW, int displayH, int imageW, int imageH) {
        LogUtils.iTag(TAG, "calculateVertexBuffer() called with: displayW = [" + displayW + "], displayH = [" + displayH + "], imageW = [" + imageW + "], imageH = [" + imageH + "]");
     ...
     * 用来计算贴纸渲染的纹理最终需要的顶点坐标
     */
    public void calculateVertexBuffer2(int displayW, int displayH, int imageW, int imageH) {
        LogUtils.iTag(TAG, "calculateVertexBuffer() called with: displayW = [" + displayW + "], displayH = [" + displayH + "], imageW = [" + imageW + "], imageH = [" + imageH + "]");
        if (displayW == 0) return;
        int outputHeight = displayH;
        int outputWidth = displayW;
        ...
        if (mVertexBuffer2 == null) {
            mVertexBuffer2 = ByteBuffer.allocateDirect(cube.length * 4)
                    .order(ByteOrder.nativeOrder())
                    .asFloatBuffer();
        }
        mVertexBuffer2.clear();
        mVertexBuffer2.put(cube).position(0);
    }
    ...
    public int preProcess3(int textureId, int width, int height) {
        if (mFrameBufferTextures2==null) {
            mFrameBufferTextures2 = new int[1];
            GLES20.glGenTextures(1, mFrameBufferTextures2, 0);
        }
        int[] mFrameBuffers2 = new int[1];
        GLES20.glGenFramebuffers(1, mFrameBuffers2, 0);
        bindFrameBuffer(mFrameBufferTextures2[0], mFrameBuffers2[0], width, height);

        if (mFrameBuffers2 == null || !mIsInitialized)
            return OpenGLUtils.NO_TEXTURE;
        ...
        int glAttribPosition = mArrayPrograms.get(0).get(POSITION_COORDINATE);
        GLES20.glVertexAttribPointer(glAttribPosition, 2, GLES20.GL_FLOAT, false, 0, mVertexBuffer2);
        GLES20.glEnableVertexAttribArray(glAttribPosition);

        mTextureBuffer2.position(0);
        int glAttribTextureCoordinate = mArrayPrograms.get(0).get(TEXTURE_COORDINATE);
        GLES20.glVertexAttribPointer(glAttribTextureCoordinate, 2, GLES20.GL_FLOAT, false, 0, mTextureBuffer2);
        GLES20.glEnableVertexAttribArray(glAttribTextureCoordinate);
        ...
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, mFrameBuffers2[0]);
        GlUtil.checkGlError("glBindFramebuffer");
        ...
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
        GLES20.glDeleteFramebuffers(1, mFrameBuffers2, 0);
        mFrameBuffers2 = null;
        GLES20.glUseProgram(0);
        return mFrameBufferTextures2[0];
    }
