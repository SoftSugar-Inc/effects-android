...
import softsugar.senseme.com.effects.databinding.ViewGradientSeekbarBinding
...
class GradientSeekbar @JvmOverloads constructor(
    ...
    private lateinit var mBinding: ViewGradientSeekbarBinding

    fun setValue(float: Float) {
        LogUtils.iTag(TAG, "setValue() called with: float = $float")
        val sb = mBinding.seekbar as TextThumbSeekBar
        sb.setValue(float)
    }
    ...
    private fun initView(context: Context) {
        mBinding = ViewGradientSeekbarBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    fun setColor(startColor: String, endColor: String) {
        mBinding.vBg.background = getGradientDrawable(startColor, endColor)
    }
    ...
    fun setProgress(float: Float) {
        val s = mBinding.seekbar as TextThumbSeekBar
        s.setValue(float)
    }
